...
class ListMusicView: UIView {
    ...
    weak var tagSongDelegate: TagButtonViewSong?
...
//        buttonAlbum.heightAnchor.constraint(equalToConstant: 10).isActive = true
        ...
        buttonAlbum.tag = tag
        buttonAlbum.addTarget(self, action: #selector(pressSong(_:)), for: .touchUpInside)
    ...
    @objc private func pressSong(_ sender: UIButton){
        tagSongDelegate?.pressButtonSong(tag: sender.tag)
...
protocol  TagButtonViewSong: AnyObject{
